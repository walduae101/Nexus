...
`;
        ...
        try {
            ...
            let xpGained = 0;
            const isMeaningful = (obj: any) => obj && typeof obj.content === 'string' && obj.content.length > 5 && !obj.content.toLowerCase().includes("no new");
            
            if (isMeaningful(distilledData.vulnerabilities_fears)) xpGained += 50;
            if (isMeaningful(distilledData.humorous_shared_jokes)) xpGained += 25;
            if (isMeaningful(distilledData.personal_goals_promises)) xpGained += 25;
            ...
            const pushHighlights = async (keyPath: string, memObj: any) => {
...
export const sweepOrphans = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
